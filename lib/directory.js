...
class DirectoryAdapter {
   ...
   */
  init () {
    return fs.promises.mkdir(this.directory).catch((err) => {
   ...
   */
  listFiles () {
    return fs.promises.readdir(this.directory).catch((err) => {
  ...
  exists (fileName) {
    return new Promise((resolve) => {
      fs.promises.access(this._resolve(fileName))
        .then(() => resolve(true))
        .catch(() => resolve(false))
    })
  }
  ...
  rename (fileName, newFileName) {
    return new Promise((resolve) => {
      ...
      resolve(fs.promises.rename(from, to))
    })
  }
  ...
  delete (fileName) {
    return new Promise((resolve) => {
      ...
      resolve(fs.promises.unlink(resolved))
    })
  }
}
